 ...
 * -----------------------------------------------------------------
 * This is the testing routine to check the SUNLinSol LpackBand
 ...
 * --------------------------------------------------------------------*)
module LapackBand_tests : (Test_linsol.LINSOL_TESTS
...
module Test = Test_linsol.Test (LapackBand_tests) (Nvector_serial.Ops)
...
let main () =
  ...
  let print_matrix_on_fail = (int_of_string Sys.argv.(4) = 1) in

  let print_timing = int_of_string Sys.argv.(5) in
  let _ = Test.set_timing (print_timing <> 0) in

  printf "@\nLapackBand linear solver test: size %d, bandwidths %d %d@\n@\n" cols uband lband;
  ...
  let ls = LS.Direct.lapack_band x mata in
  ...
  fails += Test.sunlinsolgetid ls LS.LapackBand 0;
  ...
  if !fails > 0 then begin
    ...
    if print_matrix_on_fail then begin
      ...
      printf "A (factored) =@\n%a" Matrix.Band.pp (Matrix.unwrap mata);
    end
  end else
