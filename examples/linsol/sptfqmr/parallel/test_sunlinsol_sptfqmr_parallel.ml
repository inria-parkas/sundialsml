 ...
 * --------------------------------------------------------------------*)
module SPTFQMR_tests : (Test_linsol.LINSOL_TESTS
...
module Test = Test_linsol.Test (SPTFQMR_tests) (Nvector_parallel.Ops)
...
type user_data = {
  ...
  s1     : RealArray.t;      (* scaling vectors supplied to SPTFQMR *)
  s2     : RealArray.t;
...
let main () =
  ...
  let pretype = match int_of_string Sys.argv.(2) with
  ...
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPTFQMR module failed %d initialization tests@\n@\n" !fails;
    exit 1
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPTFQMR module, problem 1, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  fails += Test.sunlinsolsetup ls None myid;
  fails += Test.sunlinsolsolve ls None xvec bvec tol true myid;
  if pretype = LS.Iterative.PrecLeft then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPTFQMR module, problem 2, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPTFQMR module, problem 3, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPTFQMR module, problem 4, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPTFQMR module, problem 5, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPTFQMR module, problem 6, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
