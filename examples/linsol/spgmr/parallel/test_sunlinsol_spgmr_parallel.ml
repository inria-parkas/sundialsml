 ...
 * --------------------------------------------------------------------*)
module SPGMR_tests : (Test_linsol.LINSOL_TESTS
...
module Test = Test_linsol.Test (SPGMR_tests) (Nvector_parallel.Ops)
...
type user_data = {
  ...
  s1     : RealArray.t;      (* scaling vectors supplied to SPGMR *)
  s2     : RealArray.t;
...
let main () =
  ...
  let pretype = match int_of_string Sys.argv.(3) with
    | 1 -> LS.Iterative.PrecLeft
    | 2 -> LS.Iterative.PrecRight
    | _ -> printf "ERROR: Preconditioning type must be either 1 or 2@\n"; exit 1
  in

  let maxl = int_of_string Sys.argv.(4) in
  if maxl <= 0 then begin
    printf "ERROR: Maximum Krylov subspace dimension must be a positive integer@\n";
    exit 1
  end;

  let tol = float_of_string Sys.argv.(5) in
  if tol <= 0.0 then begin
    printf "ERROR: Solver tolerance must be a positive real number@\n";
    exit 1
  end;

  let print_timing = int_of_string Sys.argv.(6) in
  ...
  if myid = 0 then begin
    ...
    printf "  Preconditioning type = %d@\n"
      LS.Iterative.(match pretype with PrecLeft -> 1 | PrecRight -> 2 | _ -> 0);
  ...
  let ls = LS.Iterative.spgmr ~maxl xvec in
  LS.Iterative.(set_prec_type ls pretype);

  (* Run Tests *)
  fails += Test.sunlinsolgettype ls LS.Iterative myid;
  fails += Test.sunlinsolgetid ls LS.Spgmr myid;
  ...
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPGMR module failed %d initialization tests@\n@\n" !fails;
    exit 1
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPGMR module, problem 1, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Run tests with this setup *)
  (try LS.Iterative.(set_prec_type ls pretype) with _ -> fails += 1);
  fails += Test.sunlinsolsetup ls None myid;
  fails += Test.sunlinsolsolve ls None xvec bvec tol true myid;
  fails += Test.sunlinsolsolve ls None xvec bvec tol false myid;
  fails += Test.sunlinsollastflag ls myid;
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPGMR module, problem 2, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPGMR module, problem 3, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPGMR module, problem 4, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPGMR module, problem 5, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
  ...
  (* Print result *)
  if !fails > 0 then begin
    printf "FAIL: SUNLinSol_SPGMR module, problem 6, failed %d tests@\n@\n" !fails;
    passfail += 1
  end else if myid = 0 then
