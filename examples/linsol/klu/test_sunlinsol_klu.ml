 ...
 * --------------------------------------------------------------------*)
module Klu_tests : (Test_linsol.LINSOL_TESTS
...
module Test = Test_linsol.Test (Klu_tests) (Nvector_serial.Ops)
...
let rec main () =
  ...
  if mattype' = 0
  then main_with_type n Matrix.Sparse.CSC
  else main_with_type n Matrix.Sparse.CSR

and main_with_type : type s. int -> s Matrix.Sparse.sformat -> int
  = fun n mattype ->
  let fails = ref 0 in

  printf "@\nKLU linear solver test: size %d, type %d@\n@\n"
    n (int_of_mattype mattype);
  ...
  let ls = LS.Direct.Klu.make x mata in

  (* Run Tests *)
  fails += Test.sunlinsolinitialize ls 0;
  fails += Test.sunlinsolsetup ls (Some mata) 0;
  fails += Test.sunlinsolsolve ls (Some mata) x b (1000.0 *. unit_roundoff) true 0;

  fails += Test.sunlinsolgettype ls LS.Direct 0;
  fails += Test.sunlinsolgetid ls LS.Klu 0;
  fails += Test.sunlinsollastflag ls 0;
  fails += Test.sunlinsolspace ls 0;

  (* Test 'Get' routines *)
  (* not supported:
  let symbolic = SUNLinSol_KLUGetSymbolic(lsolv) in
  if symbolic->n != n then begin
  ...
  *)
  ...
  (* not supported:
  let numeric = SUNLinSol_KLUGetNumeric(lsolv) in
  if numeric->n != n then begin
  ...
  *)
  ...
  (* not supported:
  let common = SUNLinSol_KLUGetCommon(lsolv) in
  if common->singular_col != N then begin
