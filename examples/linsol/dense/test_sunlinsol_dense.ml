 ...
 * --------------------------------------------------------------------*)
module Dense_tests : (Test_linsol.LINSOL_TESTS
...
module Test = Test_linsol.Test (Dense_tests) (Nvector_serial.Ops)
...
let main () =
  ...
  let print_on_fail =
    if Array.length Sys.argv = 4 then int_of_string Sys.argv.(3) <> 0 else false
  in

  printf "@\nDense linear solver test: size %d@\n@\n" cols;
  ...
  let ls = LS.Direct.dense x mata in
  ...
  fails += Test.sunlinsolgetid ls LS.Dense 0;
  ...
  if !fails > 0 then begin
    ...
    if print_on_fail then begin
      printf "@\nA (original) =@\n%a@\n" Matrix.Dense.pp (Matrix.unwrap matb);
      printf "A (factored) =@\n%a@\n" Matrix.Dense.pp (Matrix.unwrap mata);
      printf "x (original) =@\n%a@\n" Nvector_serial.pp y;
