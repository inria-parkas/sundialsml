...
module ArrayDense_tests =
struct
  type k = Matrix.custom
  type m = Matrix.ArrayDense.t
  type nd = Nvector_serial.data
  type nk = Nvector_serial.kind

  type t = nk Matrix.arraydense
  type nvec = Nvector_serial.t

  let rewrap = Matrix.wrap_arraydense

  let check_matrix a b tol =
    let ca, cb = Matrix.(unwrap a, unwrap b) in
    let ((ma, na) as sa) = Matrix.ArrayDense.size ca in
    let ((mb, nb) as sb) = Matrix.ArrayDense.size cb in
    let adata, bdata = Matrix.ArrayDense.(unwrap ca, unwrap cb) in
    if sa <> sb then
  ...
  let check_matrix_entry a v tol =
    ...
    let ma, na = Matrix.ArrayDense.size ca in
    let adata = Matrix.ArrayDense.unwrap ca in
    let failure = ref 0 in
    for i = 0 to ma - 1 do
      for j = 0 to na - 1 do
...
let main () =
  ...
  printf "@\nArrayDense matrix test: size %d by %d@\n@\n" matrows matcols;

  (* Create vectors and matrices *)
  let x = Nvector_serial.make matcols 0.0
  and y = Nvector_serial.make matrows 0.0
  and a = Matrix.arraydense ~m:matrows matcols
  and i = Matrix.arraydense matcols
  in

  let adata = Matrix.(ArrayDense.unwrap (unwrap a)) in
  ...
  let idata = Matrix.(ArrayDense.unwrap (unwrap i)) in
  ...
  if !fails <> 0 then begin
    ...
    printf "@\nA = %a@\n" Matrix.ArrayDense.pp (Matrix.unwrap a);
    if square then
      printf "@\nI = %a@\n" Matrix.ArrayDense.pp (Matrix.unwrap i);
    printf "@\nx = %a@\n" Nvector_serial.pp x;
    printf "@\ny = %a@\n" Nvector_serial.pp y
  end
  else
