...
let get_sol cvode_mem yy0 rtol atol tf nout proj projerr yref =
  ...
  (* Open output file *)
  let outname =
    if proj then Printf.sprintf "cvsPendulum_dns_rtol_%03.2e_rtol_%03.2e_proj.txt" rtol atol
    else Printf.sprintf "cvsPendulum_dns_rtol_%03.2e_atol_%03.2e..txt" rtol atol
  in
...
let ref_sol tf yref nout =
  ...
  (* Open output file *)
  let fid = open_out "cvsPendulum_dns_ref.txt" in
