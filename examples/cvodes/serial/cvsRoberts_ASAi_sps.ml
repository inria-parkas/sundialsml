...
let main () =
  ...
  let nnz = neq * neq in
  let cvode_mem =
    Cvode.(init BDF (Newton Sls.Superlumt.(solver_csc (jac data)
                                            ~nthreads:nthreads ~nnz:nnz))
  ...
  let cvode_memB =
    Adj.(init_backward
          cvode_mem Cvode.BDF
                    (Newton Sls.(Superlumt.solver_csc
                      (NoSens (jacb data)) ~nthreads:nthreads ~nnz:nnz))
