...
module Test =
  ...
  let make comm slen plen pglen =
    Nvector_mpimany.wrap ~comm
      (ROArray.init 2 (fun i ->
         if i = 0 then (Nvector_serial.Any.make slen 0.0)
         else (Nvector_parallel.Any.make plen pglen comm 0.0)))
...
let main () =
  ...
  if myid = 0 then begin
    ...
    printf "Vector 1 (serial) local length %d\n" loclen1;
    printf "Vector 2 (parallel) global length %d\n" globlen;
    printf "ManyVector global length %d\n" global_length;
    printf "MPI processes %d\n" nprocs
  end;

  let x = Test.make comm loclen1 loclen2 globlen in
