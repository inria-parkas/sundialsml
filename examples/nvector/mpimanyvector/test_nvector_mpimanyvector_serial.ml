...
module Test =
  ...
  let make ?with_fused_ops lens =
    ...
    Nvector_mpimany.wrap ~comm:Mpi.comm_world
      (ROArray.init num (fun i -> Nvector_serial.Any.make lens.(i) 0.0))

  let get_global_length = Nvector_mpimanyvector_ops.getlength
...
let main () =
  ...
  let glength = Test.get_global_length x in
  ...
  fails += Test.test_dotprod x y length glength 0;
  fails += Test.test_maxnorm x length 0;
  fails += Test.test_wrmsnorm x y length 0;
  fails += Test.test_wrmsnormmask x y z length glength 0;
  fails += Test.test_min x length 0;
  fails += Test.test_wl2norm x y length glength 0;
  fails += Test.test_l1norm x length glength 0;
  ...
  fails += Test.test_dotprodmulti u length glength 0;
  ...
  fails += Test.test_wrmsnormmaskvectorarray u length glength 0;
