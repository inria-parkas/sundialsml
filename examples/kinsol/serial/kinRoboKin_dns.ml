...
let jac { Kinsol.jac_u = (yd : RealArray.t); _ } j =
  ...
  (* Nonlinear equations *)
  let jd = Matrix.Dense.unwrap j in
  (*
     - 0.1238*x1 + x7 - 0.001637*x2
     - 0.9338*x4 + 0.004731*x1*x3 - 0.3578*x2*x3 - 0.3571
  *)
  jd.{0, 0} <- (- 0.1238 +. 0.004731*.x3);
  jd.{1, 0} <- (- 0.001637 -. 0.3578*.x3);
  jd.{2, 0} <- (0.004731*.x1 -. 0.3578*.x2);
  jd.{3, 0} <- (- 0.9338);
  jd.{6, 0} <- (1.0);
  ...
  *)
  jd.{0, 1} <- (0.2638 +. 0.2238*.x3);
  jd.{1, 1} <- (- 0.07745 +. 0.7623*.x3);
  jd.{2, 1} <- (0.2238*.x1 +. 0.7623*.x2);
  jd.{3, 1} <- (- 0.6734);
  jd.{6, 1} <- (-1.0);
