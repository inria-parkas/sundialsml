...
let set_ijth m i j v = m.{j - 1, i - 1} <- v
...
let jac { Kinsol.jac_u   = (yd : RealArray.t);
  ...
  *)
  set_ijth jd 1 1 (- 0.1238 +. 0.004731*.x3);
  set_ijth jd 1 2 (- 0.001637 -. 0.3578*.x3);
  set_ijth jd 1 3 (0.004731*.x1 -. 0.3578*.x2);
  set_ijth jd 1 4 (- 0.9338);
  set_ijth jd 1 7 (1.0);
  ...
  *)
  set_ijth jd 2 1 (0.2638 +. 0.2238*.x3);
  set_ijth jd 2 2 (- 0.07745 +. 0.7623*.x3);
  set_ijth jd 2 3 (0.2238*.x1 +. 0.7623*.x2);
  set_ijth jd 2 4 (- 0.6734);
  set_ijth jd 2 7 (-1.0);

  (*
    0.3578*x1 + 0.004731*x2 + x6*x8
  *)
  set_ijth jd 3 1 0.3578;
  set_ijth jd 3 2 0.004731;
  set_ijth jd 3 6 x8;
  set_ijth jd 3 8 x6;
  ...
  *)
  set_ijth jd 4 1 (- 0.7623);
  set_ijth jd 4 2 (0.2238);

  (*
    x1*x1 + x2*x2 - 1
  *)
  set_ijth jd 5 1 (2.0*.x1);
  set_ijth jd 5 2 (2.0*.x2);

  (*
    x3*x3 + x4*x4 - 1
  *)
  set_ijth jd 6 3 (2.0*.x3);
  set_ijth jd 6 4 (2.0*.x4);

  (*
    x5*x5 + x6*x6 - 1
  *)
  set_ijth jd 7 5 (2.0*.x5);
  set_ijth jd 7 6 (2.0*.x6);

  (*
    x7*x7 + x8*x8 - 1
  *)
  set_ijth jd 8 7 (2.0*.x7);
  set_ijth jd 8 8 (2.0*.x8);
   ...
   *)
  for i=1 to 8 do
    set_ijth jd (8+i) (i) (-1.0);
    set_ijth jd (8+i) (8+i) (1.0)
  done;
   ...
   *)
  for i=1 to 8 do
    set_ijth jd (16+i) i (1.0);
    set_ijth jd (16+i) (16+i) (1.0)
  done
