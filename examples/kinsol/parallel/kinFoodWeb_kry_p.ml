...
let psolvebd data
  ...
  for jx = 0 to mxsub - 1 do
    for jy = 0 to mysub - 1 do
      ...
      let pxy = data.p.(jx).(jy) in
      let piv = data.pivot.(jx).(jy) in
      Dense.getrs pxy piv vxy
    done (* end of jy loop *)
  done (* end of jx loop *)

(* Set initial conditions in cc *)
let set_initial_profiles (cc : RealArray.t) (sc : RealArray.t) =
...
let main () =
  ...
  set_initial_profiles (Nvector_parallel.unwrap cc)
                       (Nvector_parallel.unwrap sc);
  ...
  if my_pe = 0 then print_header globalstrategy maxl maxlrst fnormtol scsteptol;
