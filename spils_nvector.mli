...
(**
  The type of a function of [adata v z] that uses the information in [adata] to
  multiply a matrix A (represented internally), by the vector [v] (which must
  not be changed), storing the results in [z], and returning 0 on success, and a
  non-zero value otherwise.
 *)
type ('a, 'adata) atimes = 'adata -> 'a nvector -> 'a nvector -> int

(**
 * The type of a function of [pdata r z lr] that solves the preconditioner equation
 * [Pz = r] for the vector [z]. If [lr] is true then [P] should be taken as the
 * left preconditioner and otherwise as the right preconditioner. The function
 * should return 0 to indicate success, a positive number to indicate a
 * recoverable error, and a negative number to indicate an unrecoverable error.
 *)
type ('a, 'pdata) psolve = 'pdata -> 'a nvector -> 'a nvector -> bool -> int

(**
  [modified_gs v h k p new_vk_norm] performs a modified Gram-Schmidt
  orthogonalization  of [v[k]] against the [p] unit vectors at [v.{k-1}],
  [v.{k-2}], ..., [v.{k-p}]. Its arguments are:
  ...
  - [new_vk_norm] is filled with the Euclidean norm of the orthogonalized
    vector [v.{k}].
...
(**
  [classical_gs v h k p new_vk_norm temp s] performs a classical Gram-Schmidt
  orthogonalization  of [v[k]] against the [p] unit vectors at [v.{k-1}],
  [v.{k-2}], ..., [v.{k-p}]. Its arguments are:
  - [v] an array of [k + 1] vectors assumed to have an L2-norm of 1,
  - [h] is the output [k] by [k] Hessenberg matrix of inner products.
  ...
  sig
    ...
    *)
    ...
    (**
     ...
     [temp] indirectly specifies the prbolem size.
    ...
    (**
     [solved, nli, nps = solve s adata x b pretype gstype delta max_restarts
      pdata s1 s2 atimes psolve res_norm] solves the linear system [Ax = b]
      using the SPGMR iterative method where
      - [s] is a solver session (allocated with {!make}),
      - [adata] is information about the coefficient matrix A, passed to
        atimes,
      ...
      - [pdata] is the preconditioner information passed to [psolve],
      ...
      - [res_norm] is used for returning the L2 norm of the scaled
        preconditioned residual, [|| s1 P1_inv (b - Ax) ||_2].

      This function may throw any of the exceptions listed above. If it returns,
      then [solved] indicates whether the system converged or whether it only
      managed to reduce the norm of the residual, [nli] indicates the number of
      linear iterations performed, and [nps] indicates the number of calls made
      to [psolve].
      ...
      @raise PSolveFailure psolve failed (recoverably or not)
      @raise ATimesFailure atimes failed (recoverably or not)
      @raise PSetFailure pset failed (recoverably or not)
...
module SPBCG :
  sig
    ...
    *)
    ...
    (**
     [solved, nli, nps = solve s adata x b pretype delta pdata sx sb atimes
     psolve res_norm] solves the linear system [Ax = b] using the SPBCG
     iterative method where
      - [s] is a solver session (allocated with {!make}),
      - [adata] is information about the coefficient matrix A, passed to
        atimes.
      - [x] is the initial guess upon entry, and the solution on return,
      ...
      - [psolve] optionally solves the preconditioner system, and,
      - [res_norm] is used for returning the L2 norm of the scaled
        preconditioned residual, [||sb*P1_inv*(b-Ax)||_L2].
...
module SPTFQMR :
  sig
    ...
    *)
    ...
    (**
     [solved, nli, nps = solve s adata x b pretype delta pdata sx sb atimes
     psolve res_norm] solves the linear system [Ax = b] using the SPTFQMR
     iterative method where
      - [s] is a solver session (allocated with {!make}),
      - [adata] is information about the coefficient matrix A, passed to
        atimes.
