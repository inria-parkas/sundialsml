...
/* Note that if you call this function from C, it can throw an OCaml exception
 * so your C finalization code may not run.  */
CAMLprim void c_ida_set_linear_solver (value vida_mem, value linsolver)
{
    CAMLparam2 (vida_mem, linsolver);
    void *ida_mem = IDA_MEM_FROM_ML (vida_mem);
    int neqs = IDA_NEQS_FROM_ML (vida_mem);
    int flag;
