...
let arr_nvconst c a = Array.fill a 0 (Array.length a) c
...
let arr_nvmaxnorm =
  ...
  in Array.fold_left f 0.0

let arr_nvwrmsnorm x w =
  let f a x w = a +. ((x *. w) ** 2.0) in
  sqrt ((zip_fold_left f 0.0 x w) /. float (Array.length x))

let arr_nvwrmsnormmask x w id =
  let f a id x w =
    if id > 0.0 then a +. ((x *. w) ** 2.0) else a
  in
  sqrt ((triple_zip_fold_left f 0.0 id x w) /. float (Array.length x))

let arr_nvmin =
  let f min x = if x < min then x else min
  in Array.fold_left f max_float
...
let arr_nvinvtest x z =
  ...
  let rec f i =
    if i = l then true
    else if x.(i) = 0.0 then false
    else (z.(i) <- 1.0 /. x.(i); f (i + 1))
  in f 0
...
let array_nvec_ops = {
      Nvector.Mutable.nvclone        = Array.copy;
...
let make n e =
  Nvector.Mutable.make_nvector array_nvec_ops (Array.make n e)
