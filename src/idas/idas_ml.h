...
#define IDAS_EXN_TAG(name) (Field(Field (Field (sundials_ml_exn_table,	\
					    IDAS_EXN_SET),		\
				     IDAS_EXN_ ## name),		\
			      0))
#if OCAML_VERSION < 40200
#define IDAS_EXN(name) IDAS_EXN_TAG(name)
#else
#define IDAS_EXN(name)							\
    (Field (Field (sundials_ml_exn_table, IDAS_EXN_SET),		\
	    IDAS_EXN_ ## name))
#endif
