...
module Adjoint :
  sig (* {{{ *)
    ...
    (** Sparse Linear Solvers.
        ...
        @noidas <node> The SLS modules *)
    module Sls :
      sig (* {{{ *)

        (** Callback functions that compute sparse approximations to a Jacobian
            matrix without forward sensitivites. In the call
            [sparse_jac_fn arg jac], [arg] is a {!Idas.Adjoint.jacobian_arg}
            with three work vectors and the computed Jacobian must be stored in
            [jac].
        ...
        (** Callback functions that compute sparse approximations to a Jacobian
            matrix with forward sensitivities. In the call
            [sparse_jac_fn arg s s' jac], [arg] is a
            {!Idas.Adjoint.jacobian_arg} with three work vectors, [s] is an
            array of forward sensitivity vectors, [s'] is an array of forward
            sensitivity derivative vectors and the computed Jacobian must be
            stored in [jac].
