  ...
  struct
    ...
    module Dls =
      struct
        ...
        external c_dls_band : ('k serial_session * int) -> int -> bandrange
                                -> bool -> bool -> unit
              ...
              (match fjacfn with
               | DenseNoSens f ->
                   s.ls_callbacks <- BDlsDenseCallback { jacfn = f;
                                                         dmat = None }
               | DenseWithSens f ->
                   s.ls_callbacks <- BDlsDenseCallbackSens { jacfn_sens = f;
                                                             dmat_sens = None })
    ...
    module Sls =
      struct
        ...
        module Klu = struct
          ...
          let solver sformat f nnz bs nv =
          ...
          let get_num_jac_evals bs
            = Cvode.Sls.Klu.get_num_jac_evals (tosession bs)
        end
        ...
        module Superlumt = struct
          ...
          let solver sformat f ~nnz ~nthreads bs nv =
          ...
          let set_ordering bs
            = Cvode.Sls.Superlumt.set_ordering (tosession bs)

          let get_num_jac_evals bs
            = Cvode.Sls.Superlumt.get_num_jac_evals (tosession bs)
        end
      end
    ...
    module Spils =
      struct
        ...
        let prec_right_with_sens ?setup solve =
          InternalPrecRight (init_preconditioner_with_sens solve setup)
        let prec_both_with_sens ?setup solve =
          InternalPrecBoth (init_preconditioner_with_sens solve setup)
