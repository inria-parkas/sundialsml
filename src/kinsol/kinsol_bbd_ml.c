...
static int bbdlocal(sundials_long_int nlocal, N_Vector u, N_Vector gval, void *user_data)
{
    CAMLparam0();
    CAMLlocalN(args, 2);
 ...
 * returns void.  The comment in kinsol_bbdpre.h says it should return
 * 0 for success, non-zero otherwise.  */
static int bbdcomm(sundials_long_int nlocal, N_Vector u, void *user_data)
{
    CAMLparam0();
    CAMLlocal2(session, cb);
