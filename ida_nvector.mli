...
(** Specifies no preconditioning, with default dimension size.  See
    {!spils_params}.  *)
val spils_no_precond : 'a spils_params
  ...
  sig
    ...
    (** Set preconditioning functions (see {!spils_params}).  It may be
     ...
     val set_preconditioner :
       'a session
       -> (('a triple_tmp, 'a) jacobian_arg -> unit)
 ...
 *)
val solve_normal : 'a session -> float -> 'a nvector -> 'a nvector
  -> float * solver_result
 ...
 *)
val solve_one_step : 'a session -> float -> 'a nvector -> 'a nvector
  -> float * solver_result
 ...
 *)
val get_integrator_stats    : 'a session -> Ida.integrator_stats
...
val set_constraints : 'a session -> Constraints.t -> unit
 ...
 *)
val set_suppress_alg : 'a session -> bool -> unit
...
val set_id : 'a session -> Id.t -> unit
 ...
 *)
val set_var_types : 'a session -> VarTypes.t -> unit
 ...
 *)
val calc_ic_y : 'a session -> ?y:'a nvector -> float -> unit
...
(** [calc_ic_ya_yd' ida ~y:yvar ~y':y'var vartypes tout1] corrects the initial
    ...
    partitioned into two subsequences each (notation assumes [float array] is
    being used as nvectors):
