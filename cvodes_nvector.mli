  ...
  sig
    ...
    *)
    ...
    (** [tret = get s yq] fills [yq] with the quadrature solution vector after a
        successful return from {!Cvode_nvector.solve_normal} or
        {!Cvode_nvector.solve_one_step}, and returns the time reached by the
        solver.
    ...
    (** [tret = get_dky s t k dkyq] fills [dkyq] with the derivatives of the
        ...
        {!Cvode_nvector.solve_normal} or {!Cvode_nvector.solve_one_step}. The
        time requested, [t], must fall within the interval defined by the last
        successful step ({!Cvode_nvector.get_last_step}). The requested order,
        [k], must be less than or equal to the value returned by
        {!Cvode_nvector.get_last_order}.
...
let sp = { pvals = Some p; pbar = ...; plist = ... }]}
    ...
    (** [tret = get s ys] fills [ys] with the sensitivity solution vectors after
        a successful return from {!Cvode_nvector.solve_normal} or
        {!Cvode_nvector.solve_one_step}, and returns the time reached by the solver.
    ...
    (** [tret = get s i ys] fills [ys] with the [i]th sensitivity solution
        vector after a successful return from {!Cvode_nvector.solve_normal} or
        {!Cvode_nvector.solve_one_step}, and returns the time reached by the solver.
    ...
    (** [tret = get_dky s t k i dkys] fills [dkys] with the derivatives of the
        ...
        @raise BadK [k] is not in the range 0, 1, ..., [qlast].
        @raise BadT [t] is not in the allowed range. *)
    val get_dky1 : 'a session -> float -> int -> int -> 'a nvector -> unit
    ...
    module Quadrature :
      sig
        ...
        (** [tret = get s yqs] fills [yqs] with quadrature solution vectors
            after a successful return from {!Cvode_nvector.solve_normal} or
            {!Cvode_nvector.solve_one_step}, and returns the time reached by
            the solver.
        ...
        (**
          ...
          vector after a successful return from {!Cvode_nvector.solve_normal}
          or {!Cvode_nvector.solve_one_step}, and returns the time reached by
          the solver.
        ...
        (** [tret = get_dky s t k i dkyqs] fills [dkyqs] with the derivatives of
            ...
            {!Cvode_nvector.solve_normal} or {!Cvode_nvector.solve_one_step}.
            The time requested, [t], must fall within the interval defined by
            the last successful step ({!Cvode_nvector.get_last_step}). The
            requested order, [k], must be less than or equal to the value
            returned by {!Cvode_nvector.get_last_order}.
...
let bs = init_backward s lmm (Newton ...) (SStolerances ...) fB tB0 yB0]}
    ...
    type ('t, 'a) jacobian_arg =
      {
        ...
        jac_u   : 'a;           (** The forward solution vector. *)
        jac_ub  : 'a;           (** The backward dependent variable vector. *)
        jac_fub : 'a;           (** The backward right-hand side function fB. *)
    ...
    *)
     ...
     *)
    val get_nonlin_solv_stats : 'a bsession -> int *int
