...
type 'a linear_solver =
  ...
  *)
  ...
  (** Krylov iterative solver with the scaled preconditioned Bi-CGStab method.
      The arguments specify the preconditioner callback functions
      ({!spils_callbacks}).  See also {!Spils}.
 ...
 *)
(* TODO: remove from the nvector version. *)
and 'a band_jac_fn = ('a double_tmp, 'a) jacobian_arg
...
module Dls :
  sig
    ...
    val set_dense_jac_fn : 'a session -> 'a dense_jac_fn -> unit
    ...
    (** Change the band Jacobian function (see [Band] in {!linear_solver}).
        ...
        encouraged to use the [iter_type] parameter of {!reinit} instead,
        unless they are desperate for performance.  *)
    val set_band_jac_fn : 'a session -> 'a band_jac_fn -> unit
 ...
 *)
(* TODO: remove from the nvector version. *)
val set_no_res_mon : 'a session -> bool -> unit
 ...
 *)
(* TODO: remove from the nvector version. *)
val set_max_sub_setup_calls : 'a session -> int option -> unit
...
(**
  ...
  @kinsol <node5#ss:sysFn> Problem-defining function
 *)
val set_sys_func : 'a session -> 'a -> unit
